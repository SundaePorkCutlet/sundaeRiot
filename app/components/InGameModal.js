...
function useSpellData() {
  ...
  useEffect(() => {
    async function fetchSpellData() {
      try {
        const response = await fetch(
          'https://ddragon.leagueoflegends.com/cdn/15.3.1/data/ko_KR/summoner.json'
        );
        const data = await response.json();
                  ...
                  return (
                    <div key={player.puuid} className={styles.player}>
                      <div className={styles.championInfo}>
                        <img 
                          src={`https://ddragon.leagueoflegends.com/cdn/15.3.1/img/champion/${championName}.png`}
                          alt={`Champion ${championName}`}
                          className={styles.championImage}
                          onError={(e) => {
                            e.target.onerror = null;
                            e.target.src = 'https://ddragon.leagueoflegends.com/cdn/15.3.1/img/item/3340.png';
                          }}
                        />
                        <img 
                          src={`https://ddragon.leagueoflegends.com/cdn/15.3.1/img/spell/${spell1}.png`}
                          alt={`Spell ${spell1}`}
                          className={styles.spellImage}
                        />
                        <img 
                          src={`https://ddragon.leagueoflegends.com/cdn/15.3.1/img/spell/${spell2}.png`}
                          alt={`Spell ${spell2}`}
                          className={styles.spellImage}
                        />
                      </div>
